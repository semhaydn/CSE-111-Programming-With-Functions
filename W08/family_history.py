...
def print_death_age(people_dict):
    ...
    for person_key, person_list in people_dict.items():
            ...
            death_age = (person_list[DEATH_YEAR_INDEX]) - (person_list[BIRTH_YEAR_INDEX])
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_data in marriages_dict.items():
        ...
        husband_data = people_dict.get(husband_key)
        wife_data = people_dict.get(wife_key)

        if husband_data is not None and wife_data is not None:
            husband_name = husband_data[NAME_INDEX]
            husband_age = wedding_year - husband_data[BIRTH_YEAR_INDEX]
            wife_name = wife_data[NAME_INDEX]
            wife_age = wedding_year - wife_data[BIRTH_YEAR_INDEX]
